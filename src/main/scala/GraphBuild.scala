...
object GraphBuild {
  def main(args: Array[String]): Unit = {
    ...
    val rows = data.filter(line => header(line, "dstid") != "dstid")
    // getting only dstIds
    val dstIds = rows.map(row => header(row,"dstid"))
    // getting map from source ids to dst ids
    val sourceToDst = rows.map(row => header(row,"sourceid").toInt -> header(row,"dstid").toInt)
    ...
    val movements_rdd = sc.parallelize(movements)
      .map(feature => (feature("properties").asInstanceOf[Map[String, Any]]("MOVEMENT_ID").asInstanceOf[String].toInt,
